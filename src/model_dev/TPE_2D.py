...
from src.model_dev.Process_Raw_Input import process_concat_images_2d
from src.models.loss.loss import dice_metric, bce_dice_loss

from src.models.Unet2D_res_rnn_4L import unet2d_res_rnn_4l as model_func
...
class TPE2D:
    ...
    def __init__(self, model_func, x_train, y_train, x_test, y_test, **kwargs):
        ...
        self.RUN_NAME = kwargs.get('RUN_NAME', Cns.RUN_NAME_2D)
    ...
    def predict(self, x_test=None, y_test=None, **kwargs):
        ...
        self.model_best = self.model_func(img_rows=self.IMG_ROWS, img_cols=self.IMG_COLS, img_chns=self.IMG_CHNS, pretrained_weight_path=best_weights_path)
        print("model loaded from %s" % weight_path)
    ...
    def evaluate(self, y_test=None, y_pred=None):
        ...
        y_test_b = binarize_images(y_test, threshold=Cns.BINARY_THR)
        y_pred_b = binarize_images(y_pred, threshold=Cns.BINARY_THR)
        y_pred_best_b = binarize_images(self.y_pred_best, threshold=Cns.BINARY_THR)
...
if __name__ == '__main__':
    ...
    x_train, y_train = process_concat_images_2d(input_data_folder_adr=Cns.TRAIN_RAW_DATA_FOLDER,
                                                dtype=Cns.IMAGE_DATA_TYPE)
    x_test, y_test = process_concat_images_2d(input_data_folder_adr=Cns.TEST_RAW_DATA_FOLDER, dtype=Cns.IMAGE_DATA_TYPE)
    print('train dataset shape: %s' % str(x_train.shape))
    print('validation dataset shape: %s' % str(x_test.shape))
    print('-' * 70)

    tpe_3d_bce = TPE2D(x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test, model_func=model_func)
    tpe_3d_bce.tpe_main()
