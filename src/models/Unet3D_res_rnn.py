...
from src.models.Spatial_RNN_3D import SpatialRNN3D

# optimizer = kwargs.get('optimizer', Adam(lr=1e-5, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.000000199))
# loss = kwargs.get('loss', [bce_dice_loss])
# metrics = kwargs.get('metrics', ['accuracy'])


def unet3d_res_rnn(img_rows, img_cols, img_chns, img_depth, pretrained_weight_path=False, **kwargs):
    ...
    conv_tr_6 = Conv3DTranspose(256, (2, 2, 2), strides=(2, 2, 2), padding='same', name='conv_tr_6')(
        conc52)  # (2, 32, 32, 256)
    ...
    conv_tr_7 = Conv3DTranspose(128, (2, 2, 2), strides=(2, 2, 2), padding='same', name='conv_tr_7')(
        conc62)  # (4, 64, 64, 128)
    rnn_2d_70 = SpatialRNN3D(rnn_seq_length=32)(conv32)  # (4, 64, 64, 512)
    conv_lstm_7_1 = Bidirectional(
        ConvLSTM2D(filters=128, kernel_size=1, padding='same', return_sequences=True, name='bi_conv_lstm_8_1'))(
        rnn_2d_70)  # (4, 64, 64, 256)
    conv_lstm_7_2 = Bidirectional(
        ConvLSTM2D(filters=64, kernel_size=1, padding='same', return_sequences=True, name='bi_conv_lstm_8_2'))(
        conv_lstm_7_1)  # (4, 64, 64, 128)

    conc70 = concatenate([conv_tr_7, conv_lstm_7_2], axis=4)  # (4, 64, 64, 256)
    ...
    rnn_2d_80 = SpatialRNN3D(rnn_seq_length=64)(conv22)  # (8, 128, 128, 256)
    conv_lstm_8_1 = Bidirectional(
        ConvLSTM2D(filters=64, kernel_size=1, padding='same', return_sequences=True, name='bi_conv_lstm_8_1'))(
        rnn_2d_80)  # (8, 128, 128, 128)
    conv_lstm_8_2 = Bidirectional(
        ConvLSTM2D(filters=32, kernel_size=1, padding='same', return_sequences=True, name='bi_conv_lstm_8_2'))(
        conv_lstm_8_1)  # (8, 128, 128, 64)

    conc80 = concatenate([conv_tr_8, conv_lstm_8_2], axis=4)  # (8, 128, 128, 128)
    ...
    model = Model(inputs=[inputs], outputs=[conv10], name='Unet_Res_SRNN_32x512')
...
if __name__ == '__main__':
    model = unet3d_res_rnn(img_rows=Cns.IMG_ROWS, img_cols=Cns.IMG_COLS, img_depth=Cns.IMG_DEPTH,
                   pretrained_weight_path=False)
    model.print_summary(line_length=140)
