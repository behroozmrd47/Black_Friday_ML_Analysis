...
class SpatialRNN2D(tf.keras.layers.Layer):
    def __init__(self, rnn_seq_length, activation='relu', kernel_initializer='random_uniform', merge_mode='concat',
        ...
        y_out = spatial_rnn_concat(x_in)  # output shape of (1, 5, 5, 12)
        y_out = spatial_rnn_merge(x_in)  # output shape of (1, 5, 5, 3)
...
if __name__ == '__main__':
    ...
    i = stack_image_1
    rnn_layer = SpatialRNN2D(rnn_seq_length=1, kernel_initializer='ones')
    rnn_layer.build(input_shape=i.shape[1:])
    a = rnn_layer.call(i)
    print (a)
